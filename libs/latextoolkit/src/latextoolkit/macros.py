...
def create_latex_command(directory, name, value):
    ...
    name = _sanitize_command_name(name)
...
def _sanitize_command_name(name):
    """Sanitize the command name by replacing digits with words and removing underscores."""
    # Replace digits with words (1 -> one, 2 -> two, etc.)
    name = _replace_digits(name)
    # Underscores are not allowed in latex commands
    name = _remove_underscores(name)
